...
from Calculadora import soma, subtracao, multiplicacao, divisao, exponenciacao, calculadora
...
def test_calculadora_soma(monkeypatch, capsys):
    inputs = iter(["1", "2", "3"])
...
def test_exponenciacao_inteiros(numeros_inteiros):
    inteiros = numeros_inteiros

    assert exponenciacao(inteiros[0], inteiros[1]) == 243  # 3^5 = 243
    assert exponenciacao(inteiros[3], inteiros[0]) == 1000  # 10^3 = 1000
    assert exponenciacao(inteiros[4], inteiros[5]) == 0  # 0^100 = 0
    assert exponenciacao(inteiros[1], inteiros[4]) == 1  # 5^0 = 1


def test_exponenciacao_float(numeros_ponto_flutuante):
    flutuantes = numeros_ponto_flutuante
    
    assert exponenciacao(flutuantes[0], flutuantes[1]) == pytest.approx(22.9176, abs=0.001)  # 3.5^2.5
    assert exponenciacao(flutuantes[1], flutuantes[0]) == pytest.approx(24.7054, abs=0.001)  # 2.5^3.5
    assert exponenciacao(flutuantes[3], flutuantes[4]) == pytest.approx(1.0414, abs=0.001)  # 1.5^0.1
    assert exponenciacao(flutuantes[4], flutuantes[5]) == pytest.approx(0.631, abs=0.001)  # -1.5^1.5


def test_calculadora_exponenciacao(monkeypatch, capsys):
    inputs = iter(["5", "2", "3"])  # 2^3 = 8
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
    calculadora()
    captured = capsys.readouterr()
    assert "Resultado: 8.0" in captured.out