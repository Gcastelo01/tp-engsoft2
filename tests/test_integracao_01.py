...
class TestSistemaGerenciamentoAcesso(unittest.TestCase):
    def setUp(self):
        ...
        self.funcionalidade5 = Funcionalidades(
            id=5, nome="Calculadora de Equações", nivel=2
        )
    ...
    def test_cadastro_usuario_associacao_funcionalidades(self):
        ...
        funcionalidades_esperadas = [
            ...
            5,
            6,
        ]  # IDs das funcionalidades que devem ser liberadas

        funcionalidades_associadas = self.sistema.mostrarFuncionalidadesLiberadas(
            novo_usuario.id
        )

        self.assertEqual(
            set(funcionalidades_esperadas), set(funcionalidades_associadas)
        )
